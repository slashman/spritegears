...
public class SwingSystemInterface implements Runnable{ 
	...
	public void printAtPixelCentered(int x, int y, String string, Color color) {
		int width = getStringWidth(string);
		printAtPixel(x - width/2, y, string, color);
	}
	
	public int getStringWidth(String string){
		return sip.getFontMetrics(sip.getFontFace()).stringWidth(string);
	}
	...
	public void setZOrder(Component component, int zOrder) {
		sip.setComponentZOrder(component, zOrder);
	}

	public void showPopupMenu(JPopupMenu test, int x, int y) {
		test.show(sip, x, y);
	}

	public void showMessage(String string) {
		JOptionPane.showMessageDialog(sip, string);		
	}
	

	public boolean showMessagePrompt(String string) {
		return JOptionPane.showConfirmDialog(sip, string) == JOptionPane.YES_OPTION;
	}
